...
static void blink_work_handler(struct k_work *work)
{
    ...
    if (!data->blinking_active) {
        LOG_DBG("Blinking not active, ignoring");
        return;
    }
    ...
    if (data->blinking_active) {
        ...
        LOG_DBG("Scheduled next blink in %u ms", data->blink_interval_ms);
    }
}
...
static void start_blinking(struct led_controller_data *data, uint32_t interval_ms)
{
    ...
    LOG_INF("Starting blinking with interval %u ms", interval_ms);
    ...
    data->led_on = true;
    set_gpio_level(true);
    LOG_DBG("Initial LED ON");
    
    // 关键：调度第一次熄灭（间隔为interval_ms）
    k_work_reschedule(&data->blink_work, K_MSEC(interval_ms));
    LOG_DBG("Scheduled first toggle in %u ms", interval_ms);
}
